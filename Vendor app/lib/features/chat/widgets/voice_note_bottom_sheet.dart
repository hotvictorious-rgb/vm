  ...
  Future<void> _startRecording() async {
    try {
      if (await _audioRecorder.hasPermission()) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(getTranslated('microphone_permission_denied', context) ?? 'Microphone permission denied')));
      }
    } catch (e) {
    ...
    return Container(
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: Theme.of(context).hintColor.withOpacity(0.5), borderRadius: BorderRadius.circular(2))),
            const SizedBox(height: Dimensions.paddingSizeLarge),
            
            if (!_hasRecorded) ...[
              Text(
                _isRecording ? _formatDuration(_recordDuration) : (getTranslated('record_voice_note', context) ?? 'Record Voice Note'),
                style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeExtraLarge, color: _isRecording ? Colors.red : null),
              ),
              ...
              Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: CircleAvatar(
                      radius: 40,
                      backgroundColor: _isRecording ? Theme.of(context).colorScheme.error : Theme.of(context).primaryColor,
              ...
              Row(
                children: [
                  IconButton(
                    ...
                    icon: Icon(_isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled, size: 40, color: Theme.of(context).primaryColor),
                  ),
                  Expanded(
                    child: Slider(
              ...
              Row(
                ...
                children: [
                  TextButton.icon(
                    ...
                    label: Text(getTranslated('delete', context) ?? 'Delete', style: robotoRegular.copyWith(color: Theme.of(context).colorScheme.error)),
                  ),
                  ElevatedButton.icon(
                    onPressed: _sendVoiceNote,
                    style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).primaryColor),
                    icon: const Icon(Icons.send, color: Colors.white),
                    label: Text(getTranslated('send', context) ?? 'Send', style: robotoMedium.copyWith(color: Colors.white)),
                  )
                ],
              )
            ],
